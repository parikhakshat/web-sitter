...
pub use super::javascript::{
    JS_DB_SINKS, JS_DESERIALIZATION_SINKS, JS_DOM_XSS_SINKS, JS_EVAL_SINKS, JS_EXEC_SINKS,
    JS_FILE_WRITE_SINKS, JS_LDAP_SINKS, JS_REDIRECT_SINKS, JS_SSRF_SINKS, JS_TAINT_PROPAGATORS,
    JS_TAINT_SINKS, JS_TAINT_SOURCES, JS_TEMPLATE_SINKS, JS_VM_SINKS,
};
...
pub const TS_TAINT_SINKS: &[(&str, SinkSpec)] = &[
    ...
    ("createQueryBuilder", SinkSpec { sink_args: &[0] }),
    ("where", SinkSpec { sink_args: &[0] }),
    ("andWhere", SinkSpec { sink_args: &[0] }),
    ("orWhere", SinkSpec { sink_args: &[0] }),
    ("having", SinkSpec { sink_args: &[0] }),
    ("select", SinkSpec { sink_args: &[0] }),
    ("orderBy", SinkSpec { sink_args: &[0] }),
    // ── Prisma — $queryRaw / $executeRaw allow SQL injection with template ────
    ("$queryRaw", SinkSpec { sink_args: &[0] }),
    ("$executeRaw", SinkSpec { sink_args: &[0] }),
    ("$queryRawUnsafe", SinkSpec { sink_args: &[0] }),
    ("$executeRawUnsafe", SinkSpec { sink_args: &[0] }),
    // ── Angular — DomSanitizer bypass — XSS ──────────────────────────────────
    ("bypassSecurityTrustHtml", SinkSpec { sink_args: &[0] }),
    ("bypassSecurityTrustScript", SinkSpec { sink_args: &[0] }),
    ("bypassSecurityTrustStyle", SinkSpec { sink_args: &[0] }),
    ("bypassSecurityTrustUrl", SinkSpec { sink_args: &[0] }),
    (
        "bypassSecurityTrustResourceUrl",
        SinkSpec { sink_args: &[0] },
    ),
    // ── axios / superagent / got — SSRF ──────────────────────────────────────
    ("axios", SinkSpec { sink_args: &[0] }),
    ...
    ("forRoot", SinkSpec { sink_args: &[0] }),
    ("forRootAsync", SinkSpec { sink_args: &[0] }),
    // ── GraphQL — raw query strings ───────────────────────────────────────────
    ("gql", SinkSpec { sink_args: &[0] }),
...
pub const TS_TAINT_PROPAGATORS: &[(&str, PropagatorSpec)] = &[
    ...
    ("transform", PropagatorSpec { dst: -1, src: &[] }),
    ("refine", PropagatorSpec { dst: -1, src: &[] }),
    ("parse", PropagatorSpec { dst: -1, src: &[0] }),
    // Angular Pipe.transform
    ("transform", PropagatorSpec { dst: -1, src: &[0] }),
