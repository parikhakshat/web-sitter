...
pub const GO_TAINT_SINKS: &[(&str, SinkSpec)] = &[
    ...
    ("CommandContext", SinkSpec { sink_args: &[1] }),
    // (*exec.Cmd).Run / Output / CombinedOutput execute the command
    ("Run", SinkSpec { sink_args: &[] }),
    ("Output", SinkSpec { sink_args: &[] }),
    ("CombinedOutput", SinkSpec { sink_args: &[] }),
    ("Start", SinkSpec { sink_args: &[] }),
    ("Wait", SinkSpec { sink_args: &[] }),
    // syscall.Exec / syscall.ForkExec
    ("Exec", SinkSpec { sink_args: &[0, 1] }),
    ("ForkExec", SinkSpec { sink_args: &[0, 1] }),
    // ── database/sql — SQL injection ──────────────────────────────────────────
    ("Query", SinkSpec { sink_args: &[0] }),
    ("QueryRow", SinkSpec { sink_args: &[0] }),
    ("QueryContext", SinkSpec { sink_args: &[1] }),
    ("QueryRowContext", SinkSpec { sink_args: &[1] }),
    ("Prepare", SinkSpec { sink_args: &[0] }),
    ("PrepareContext", SinkSpec { sink_args: &[1] }),
    // ── text/template — template injection (use html/template for HTML) ────────
    // template.Execute with user-controlled template name
    ("Execute", SinkSpec { sink_args: &[1] }),
    ("ExecuteTemplate", SinkSpec { sink_args: &[1, 2] }),
    ("ParseFiles", SinkSpec { sink_args: &[0] }),
    ("ParseGlob", SinkSpec { sink_args: &[0] }),
    ("ParseFS", SinkSpec { sink_args: &[0, 1] }),
    // ── html/template — unsafe conversions (bypass HTML escaping) ─────────────
    // template.HTML(s), template.JS(s), template.URL(s) etc.
    ("HTML", SinkSpec { sink_args: &[0] }),
    ("JS", SinkSpec { sink_args: &[0] }),
    ("URL", SinkSpec { sink_args: &[0] }),
    ("Attr", SinkSpec { sink_args: &[0] }),
    ("CSS", SinkSpec { sink_args: &[0] }),
    ("JSStr", SinkSpec { sink_args: &[0] }),
    // ── os — file system path traversal ──────────────────────────────────────
    ("Open", SinkSpec { sink_args: &[0] }),
    ("OpenFile", SinkSpec { sink_args: &[0] }),
    ("Create", SinkSpec { sink_args: &[0] }),
    ("CreateTemp", SinkSpec { sink_args: &[0, 1] }),
    ("Mkdir", SinkSpec { sink_args: &[0] }),
    ("MkdirAll", SinkSpec { sink_args: &[0] }),
    ("MkdirTemp", SinkSpec { sink_args: &[0, 1] }),
    ("Remove", SinkSpec { sink_args: &[0] }),
    ("RemoveAll", SinkSpec { sink_args: &[0] }),
    ("Rename", SinkSpec { sink_args: &[0, 1] }),
    ("Symlink", SinkSpec { sink_args: &[0, 1] }),
    ("Link", SinkSpec { sink_args: &[0, 1] }),
    ("WriteFile", SinkSpec { sink_args: &[0, 1] }),
    ...
    ("NewRequest", SinkSpec { sink_args: &[1] }),
    ("NewRequestWithContext", SinkSpec { sink_args: &[2] }),
    ("ServeFile", SinkSpec { sink_args: &[2] }),
    ("ServeContent", SinkSpec { sink_args: &[2] }),
    ("Redirect", SinkSpec { sink_args: &[2] }),
    // http.ResponseWriter.Header().Set() — header injection
    ("Set", SinkSpec { sink_args: &[0, 1] }),
    ("Add", SinkSpec { sink_args: &[0, 1] }),
    // ── net — network dial (SSRF) ─────────────────────────────────────────────
    ("Dial", SinkSpec { sink_args: &[0, 1] }),
    ("DialContext", SinkSpec { sink_args: &[1, 2] }),
    ("DialTimeout", SinkSpec { sink_args: &[0, 1] }),
    (
        "DialTCP",
        SinkSpec {
            sink_args: &[0, 1, 2],
    ...
    ("LookupIP", SinkSpec { sink_args: &[0] }),
    ("LookupAddr", SinkSpec { sink_args: &[0] }),
    // ── log — log injection ────────────────────────────────────────────────────
    ("Print", SinkSpec { sink_args: &[0] }),
    ("Println", SinkSpec { sink_args: &[0] }),
    ("Printf", SinkSpec { sink_args: &[0] }),
    ("Fatal", SinkSpec { sink_args: &[0] }),
    ("Fatalf", SinkSpec { sink_args: &[0] }),
    ("Fatalln", SinkSpec { sink_args: &[0] }),
    // ── fmt — formatted I/O write sinks ──────────────────────────────────────
    (
        "Fprintf", // fmt.Fprintf(w, format, ...) — write to http.ResponseWriter
        SinkSpec { sink_args: &[1] },
    ),
    ("Fprintln", SinkSpec { sink_args: &[1] }),
    ("Fprint", SinkSpec { sink_args: &[1] }),
    // ── LDAP (various Go LDAP packages) ───────────────────────────────────────
    ("Search", SinkSpec { sink_args: &[0] }),
    // ── go-redis / MongoDB Go driver ──────────────────────────────────────────
    ("Do", SinkSpec { sink_args: &[0] }),
    ("FindOne", SinkSpec { sink_args: &[1] }),
    ("Find", SinkSpec { sink_args: &[1] }),
    ("UpdateOne", SinkSpec { sink_args: &[1, 2] }),
    ("DeleteOne", SinkSpec { sink_args: &[1] }),
    // ── url.Parse — SSRF input validation bypass ──────────────────────────────
    (
        "Parse", // url.Parse — attacker supplies URL
        SinkSpec { sink_args: &[0] },
    ),
    ("ParseRequestURI", SinkSpec { sink_args: &[0] }),
];
...
pub const GO_TAINT_PROPAGATORS: &[(&str, PropagatorSpec)] = &[
    ...
    ("ToLower", PropagatorSpec { dst: -1, src: &[0] }),
    ("ToUpper", PropagatorSpec { dst: -1, src: &[0] }),
    ("ToTitle", PropagatorSpec { dst: -1, src: &[0] }),
    ("TrimSpace", PropagatorSpec { dst: -1, src: &[0] }),
    ("Trim", PropagatorSpec { dst: -1, src: &[0] }),
    ("TrimLeft", PropagatorSpec { dst: -1, src: &[0] }),
    ("TrimRight", PropagatorSpec { dst: -1, src: &[0] }),
    ("TrimPrefix", PropagatorSpec { dst: -1, src: &[0] }),
    ("TrimSuffix", PropagatorSpec { dst: -1, src: &[0] }),
    ("TrimFunc", PropagatorSpec { dst: -1, src: &[0] }),
    ("Split", PropagatorSpec { dst: -1, src: &[0] }),
    ("SplitN", PropagatorSpec { dst: -1, src: &[0] }),
    ("SplitAfter", PropagatorSpec { dst: -1, src: &[0] }),
    ("SplitAfterN", PropagatorSpec { dst: -1, src: &[0] }),
    ("Fields", PropagatorSpec { dst: -1, src: &[0] }),
    ("FieldsFunc", PropagatorSpec { dst: -1, src: &[0] }),
    ("Repeat", PropagatorSpec { dst: -1, src: &[0] }),
    ("Map", PropagatorSpec { dst: -1, src: &[1] }),
    (
        "NewReplacer",
        PropagatorSpec {
            dst: -1,
    ...
    ("WriteString", PropagatorSpec { dst: -1, src: &[0] }),
    ("Write", PropagatorSpec { dst: -1, src: &[0] }),
    ("WriteByte", PropagatorSpec { dst: -1, src: &[0] }),
    ("WriteRune", PropagatorSpec { dst: -1, src: &[0] }),
    // strings.Builder.String() — returns the built string
    ("String", PropagatorSpec { dst: -1, src: &[] }),
    // ── bytes ─────────────────────────────────────────────────────────────────
    ("Join", PropagatorSpec { dst: -1, src: &[0] }),
    (
        "Replace",
    ...
    ("Base", PropagatorSpec { dst: -1, src: &[0] }),
    ("Dir", PropagatorSpec { dst: -1, src: &[0] }),
    ("Ext", PropagatorSpec { dst: -1, src: &[0] }),
    ("Clean", PropagatorSpec { dst: -1, src: &[0] }),
    ("EvalSymlinks", PropagatorSpec { dst: -1, src: &[0] }),
    ("Abs", PropagatorSpec { dst: -1, src: &[0] }),
    ("Rel", PropagatorSpec { dst: -1, src: &[1] }),
    // ── strconv ───────────────────────────────────────────────────────────────
    ("Itoa", PropagatorSpec { dst: -1, src: &[0] }),
    ("FormatInt", PropagatorSpec { dst: -1, src: &[0] }),
    ("FormatFloat", PropagatorSpec { dst: -1, src: &[0] }),
    ("FormatBool", PropagatorSpec { dst: -1, src: &[0] }),
    ("Quote", PropagatorSpec { dst: -1, src: &[0] }),
    ("Unquote", PropagatorSpec { dst: -1, src: &[0] }),
    (
        "AppendInt",
        PropagatorSpec {
            dst: -1,
    ...
    ("QueryEscape", PropagatorSpec { dst: -1, src: &[0] }),
    ("PathEscape", PropagatorSpec { dst: -1, src: &[0] }),
    ("QueryUnescape", PropagatorSpec { dst: -1, src: &[0] }),
    ("PathUnescape", PropagatorSpec { dst: -1, src: &[0] }),
    // ── encoding/base64 — encoding does not sanitize taint ────────────────────
    ("EncodeToString", PropagatorSpec { dst: -1, src: &[0] }),
    ("DecodeString", PropagatorSpec { dst: -1, src: &[0] }),
    ("Encode", PropagatorSpec { dst: -1, src: &[1] }),
    ("Decode", PropagatorSpec { dst: -1, src: &[1] }),
    // ── encoding/json — taint passes through marshaling ───────────────────────
    ("Marshal", PropagatorSpec { dst: -1, src: &[0] }),
    ("Unmarshal", PropagatorSpec { dst: -1, src: &[0] }),
    ...
    ("FindString", PropagatorSpec { dst: -1, src: &[0] }),
    ("FindAllString", PropagatorSpec { dst: -1, src: &[0] }),
    ("FindStringSubmatch", PropagatorSpec { dst: -1, src: &[0] }),
];
...
pub const GO_ENV_SOURCES: &[&str] = &["Getenv", "LookupEnv", "Expand", "ExpandEnv", "Environ"];
