...
pub const JS_TAINT_SINKS: &[(&str, SinkSpec)] = &[
    ...
    ("Function", SinkSpec { sink_args: &[0] }),
    // setTimeout / setInterval with a string argument
    ("setTimeout", SinkSpec { sink_args: &[0] }),
    ("setInterval", SinkSpec { sink_args: &[0] }),
    ("setImmediate", SinkSpec { sink_args: &[0] }),
    // ── OS command injection — child_process ──────────────────────────────────
    ("exec", SinkSpec { sink_args: &[0] }),
    ("execSync", SinkSpec { sink_args: &[0] }),
    ("spawn", SinkSpec { sink_args: &[0] }),
    ("spawnSync", SinkSpec { sink_args: &[0] }),
    ("execFile", SinkSpec { sink_args: &[0] }),
    ("execFileSync", SinkSpec { sink_args: &[0] }),
    ("fork", SinkSpec { sink_args: &[0] }),
    // ── File operations — path traversal / file write ─────────────────────────
    ("writeFile", SinkSpec { sink_args: &[0, 1] }),
    ("writeFileSync", SinkSpec { sink_args: &[0, 1] }),
    ("appendFile", SinkSpec { sink_args: &[0, 1] }),
    ("appendFileSync", SinkSpec { sink_args: &[0, 1] }),
    ("unlink", SinkSpec { sink_args: &[0] }),
    ("unlinkSync", SinkSpec { sink_args: &[0] }),
    ("rmdir", SinkSpec { sink_args: &[0] }),
    ("rmdirSync", SinkSpec { sink_args: &[0] }),
    ("mkdir", SinkSpec { sink_args: &[0] }),
    ("mkdirSync", SinkSpec { sink_args: &[0] }),
    ("rename", SinkSpec { sink_args: &[0, 1] }),
    ("renameSync", SinkSpec { sink_args: &[0, 1] }),
    ("copyFile", SinkSpec { sink_args: &[0, 1] }),
    ("copyFileSync", SinkSpec { sink_args: &[0, 1] }),
    ("open", SinkSpec { sink_args: &[0] }),
    ("openSync", SinkSpec { sink_args: &[0] }),
    ("createWriteStream", SinkSpec { sink_args: &[0] }),
    ("createReadStream", SinkSpec { sink_args: &[0] }),
    ...
    ("writeln", SinkSpec { sink_args: &[0] }),
    ("insertAdjacentHTML", SinkSpec { sink_args: &[1] }),
    ("createContextualFragment", SinkSpec { sink_args: &[0] }),
    ("setAttribute", SinkSpec { sink_args: &[0, 1] }),
    (
        "setAttributeNS",
        SinkSpec {
            sink_args: &[0, 1, 2],
    ...
    ("append", SinkSpec { sink_args: &[0] }),
    ("prepend", SinkSpec { sink_args: &[0] }),
    ("after", SinkSpec { sink_args: &[0] }),
    ("before", SinkSpec { sink_args: &[0] }),
    ("replaceWith", SinkSpec { sink_args: &[0] }),
    ("wrap", SinkSpec { sink_args: &[0] }),
    ("wrapAll", SinkSpec { sink_args: &[0] }),
    ("wrapInner", SinkSpec { sink_args: &[0] }),
    // ── SQL injection — various Node.js database libraries ────────────────────
    ("query", SinkSpec { sink_args: &[0] }),
    ("run", SinkSpec { sink_args: &[0] }),
    ("all", SinkSpec { sink_args: &[0] }),
    ("get", SinkSpec { sink_args: &[0] }),
    ("execute", SinkSpec { sink_args: &[0] }),
    ("raw", SinkSpec { sink_args: &[0] }),
    // Knex .whereRaw / .havingRaw
    ("whereRaw", SinkSpec { sink_args: &[0] }),
    ("havingRaw", SinkSpec { sink_args: &[0] }),
    ("orderByRaw", SinkSpec { sink_args: &[0] }),
    ("groupByRaw", SinkSpec { sink_args: &[0] }),
    // Mongoose / MongoDB — NoSQL injection
    ("find", SinkSpec { sink_args: &[0] }),
    ("findOne", SinkSpec { sink_args: &[0] }),
    ("findById", SinkSpec { sink_args: &[0] }),
    ("findOneAndUpdate", SinkSpec { sink_args: &[0, 1] }),
    ("findOneAndDelete", SinkSpec { sink_args: &[0] }),
    ("updateOne", SinkSpec { sink_args: &[0, 1] }),
    ("deleteOne", SinkSpec { sink_args: &[0] }),
    // ── SSRF — HTTP requests ──────────────────────────────────────────────────
    ("fetch", SinkSpec { sink_args: &[0] }),
    ("request", SinkSpec { sink_args: &[0] }),
    ("get", SinkSpec { sink_args: &[0] }),
    ("post", SinkSpec { sink_args: &[0] }),
    ("put", SinkSpec { sink_args: &[0] }),
    ("delete", SinkSpec { sink_args: &[0] }),
    // ── Open redirect — Express / Koa ─────────────────────────────────────────
    ("redirect", SinkSpec { sink_args: &[0] }),
    // ── Template injection ────────────────────────────────────────────────────
    ("render", SinkSpec { sink_args: &[0, 1] }),
    ("renderFile", SinkSpec { sink_args: &[0] }),
    ("compile", SinkSpec { sink_args: &[0] }),
    ("template", SinkSpec { sink_args: &[0] }),
    // Pug / Jade
    ("renderSync", SinkSpec { sink_args: &[0] }),
    // ── LDAP injection — ldapjs ───────────────────────────────────────────────
    ("search", SinkSpec { sink_args: &[0, 1] }),
    // ── Path traversal helpers ────────────────────────────────────────────────
    ("sendFile", SinkSpec { sink_args: &[0] }),
    ("download", SinkSpec { sink_args: &[0] }),
    ("pipe", SinkSpec { sink_args: &[0] }),
    // ── Node.js vm module — sandbox escape ────────────────────────────────────
    ("runInNewContext", SinkSpec { sink_args: &[0] }),
    ("runInContext", SinkSpec { sink_args: &[0] }),
    ("runInThisContext", SinkSpec { sink_args: &[0] }),
    ("Script", SinkSpec { sink_args: &[0] }),
    // ── node-serialize / serialize-javascript — deserialization RCE ───────────
    ("unserialize", SinkSpec { sink_args: &[0] }),
    ("deserialize", SinkSpec { sink_args: &[0] }),
];
...
pub const JS_TAINT_PROPAGATORS: &[(&str, PropagatorSpec)] = &[
    ...
    ("slice", PropagatorSpec { dst: -1, src: &[] }),
    ("substring", PropagatorSpec { dst: -1, src: &[] }),
    ("substr", PropagatorSpec { dst: -1, src: &[] }),
    ...
    ("toLocaleLowerCase", PropagatorSpec { dst: -1, src: &[] }),
    ("toLocaleUpperCase", PropagatorSpec { dst: -1, src: &[] }),
    ("trim", PropagatorSpec { dst: -1, src: &[] }),
    ("trimStart", PropagatorSpec { dst: -1, src: &[] }),
    ("trimEnd", PropagatorSpec { dst: -1, src: &[] }),
    ("padStart", PropagatorSpec { dst: -1, src: &[1] }),
    ("padEnd", PropagatorSpec { dst: -1, src: &[1] }),
    ("repeat", PropagatorSpec { dst: -1, src: &[] }),
    ("split", PropagatorSpec { dst: -1, src: &[0] }),
    ("at", PropagatorSpec { dst: -1, src: &[] }),
    ("charAt", PropagatorSpec { dst: -1, src: &[] }),
    ("charCodeAt", PropagatorSpec { dst: -1, src: &[] }),
    ("codePointAt", PropagatorSpec { dst: -1, src: &[] }),
    ("toString", PropagatorSpec { dst: -1, src: &[] }),
    ("valueOf", PropagatorSpec { dst: -1, src: &[] }),
    ("normalize", PropagatorSpec { dst: -1, src: &[] }),
    // ── Array methods ─────────────────────────────────────────────────────────
    ("join", PropagatorSpec { dst: -1, src: &[0] }),
    ("map", PropagatorSpec { dst: -1, src: &[] }),
    ("filter", PropagatorSpec { dst: -1, src: &[] }),
    ("reduce", PropagatorSpec { dst: -1, src: &[] }),
    ("reduceRight", PropagatorSpec { dst: -1, src: &[] }),
    ("flat", PropagatorSpec { dst: -1, src: &[] }),
    ...
    ("at", PropagatorSpec { dst: -1, src: &[] }),
    ("find", PropagatorSpec { dst: -1, src: &[] }),
    ("findIndex", PropagatorSpec { dst: -1, src: &[] }),
    ("sort", PropagatorSpec { dst: -1, src: &[] }),
    ...
    ("stringify", PropagatorSpec { dst: -1, src: &[0] }),
    // ── URL / path manipulation — taint passes through ────────────────────────
    ("encodeURIComponent", PropagatorSpec { dst: -1, src: &[0] }),
    ("encodeURI", PropagatorSpec { dst: -1, src: &[0] }),
    ("decodeURIComponent", PropagatorSpec { dst: -1, src: &[0] }),
    ("decodeURI", PropagatorSpec { dst: -1, src: &[0] }),
    ("btoa", PropagatorSpec { dst: -1, src: &[0] }),
    ("atob", PropagatorSpec { dst: -1, src: &[0] }),
    // ── path (Node.js) ────────────────────────────────────────────────────────
    (
        "join",
        PropagatorSpec {
    ...
    ("normalize", PropagatorSpec { dst: -1, src: &[0] }),
    ("basename", PropagatorSpec { dst: -1, src: &[0] }),
    ("dirname", PropagatorSpec { dst: -1, src: &[0] }),
    ("extname", PropagatorSpec { dst: -1, src: &[0] }),
    // ── Buffer (Node.js) ─────────────────────────────────────────────────────
    ("toString", PropagatorSpec { dst: -1, src: &[] }),
    ("from", PropagatorSpec { dst: -1, src: &[0] }),
];
...
pub const JS_SSRF_SINKS: &[&str] = &["fetch", "request", "get", "post", "put", "delete"];
