...
impl CpgGenerator {
    ...
    pub fn generate_from_file_with_options(
        ...
        // Single source of truth for macro-alias resolution: both function-like
        // wrapper macros (`__macro_alias_` prefix) and plain object-like renames
        // (`#define ngx_free free` — stored under the bare macro name, no prefix,
        // since `extract_macros` only prefixes the params+body dump for those).
        // `options.macro_aliases` used to filter on the `__macro_alias_` prefix
        // alone, silently dropping every object-like rename (e.g. nginx's
        // `ngx_free` → `free`) from call/taint resolution even though the same
        // data was captured correctly here for `cpg.c_file.macro_aliases`.
        let orig_case_macro_aliases: BTreeMap<String, String> = raw_macros
            ...
            .collect();
        let macro_aliases: BTreeMap<String, String> = orig_case_macro_aliases
            .iter()
...
/// Find the name of the function actually called in a macro body, skipping
/// over a leading parenthesized cast. Wrapper macros commonly look like
/// `(void) memcpy(dst, src, n)` (nginx's own style, used to suppress
/// "return value ignored" warnings) or `((u_char *) memcpy(dst, src, n)) + n`
/// — naively taking the text before the body's first `(` would grab the
/// cast's own parenthesis and return an empty/wrong name instead of the
/// call actually being wrapped. Scanning for the first `identifier(`
/// occurrence anywhere in the body finds the real call regardless of any
/// cast prefix.
fn find_wrapped_call_name(body: &str) -> Option<String> {
    static CALL_RE: std::sync::OnceLock<regex::Regex> = std::sync::OnceLock::new();
    let re = CALL_RE.get_or_init(|| regex::Regex::new(r"([A-Za-z_][A-Za-z0-9_]*)\s*\(").unwrap());
    re.captures(body).map(|c| c[1].to_string())
}

/// Discover a bounded set of include directories for `file_path`'s project, so
/// `gcc -E`-based macro extraction can resolve `#include "..."` headers that
/// live in sibling directories rather than next to the source file itself
/// (e.g. nginx's `src/core/ngx_config.h`, included from `src/http/*.c`).
/// Without this, `gcc` was invoked with no `-I` flags at all, so it failed
/// outright (`fatal error: ... No such file or directory`) on essentially any
/// multi-directory C project, returning an empty macro table — silently
/// disabling macro-alias resolution (`ngx_memcpy` → `memcpy`, `ngx_free` →
/// `free`, etc.) for the whole file.
///
/// Walks up from the file looking for a project-root marker (`.git`,
/// `configure`, `Makefile`, `CMakeLists.txt`), then recursively collects every
/// subdirectory under that root (bounded, to avoid scanning an unrelated huge
/// tree) — this needs no build-system integration to work out of the box, at
/// the cost of not resolving headers generated by a project's own build step
/// (e.g. autoconf's `ngx_auto_config.h`) that don't exist in a plain checkout.
fn discover_include_dirs(file_path: &Path) -> Vec<PathBuf> {
    const MAX_DIRS: usize = 512;
    const MAX_ANCESTOR_HOPS: usize = 8;

    let file_dir = file_path.parent().unwrap_or_else(|| Path::new("."));
    let mut root = file_dir.to_path_buf();
    for _ in 0..MAX_ANCESTOR_HOPS {
        if [".git", "configure", "Makefile", "CMakeLists.txt"]
            .iter()
    ...
    while let Some(dir) = stack.pop() {
        ...
        for entry in entries.flatten() {
            ...
            if name.starts_with('.') || matches!(name.as_ref(), "target" | "node_modules" | "vendor") {
                continue;
            }
...
pub fn extract_macros(file_path: impl AsRef<Path>) -> BTreeMap<String, String> {
    ...
    let include_dirs = discover_include_dirs(file_path.as_ref());
    let mut cmd = Command::new("gcc");
    cmd.arg("-dM").arg("-E").arg("-x").arg("c");
    ...
    for line in stdout.lines() {
        ...
        if let Some(paren) = rest.find('(') {
            ...
            if !name.is_empty() {
                ...
                if let Some(called) = find_wrapped_call_name(body) {
                    if called != name
                        && !matches!(called.as_str(), "if" | "while" | "for" | "return" | "sizeof" | "do")
                    {
                        macros.insert(format!("__macro_alias_{name}"), called);
                    }
                }
